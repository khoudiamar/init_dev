...
/*
	NOTE : when -git or -help are chosen as options 
	their value becomes 1 (git == 1) ()
*/
...
int params_are_correct(){
	// nbr d'argumets doit etre >= 2 args = [<nom du fichier>, nom_projet, ... autres parametres ... ]
	if (nbr_arguments < 2)
	{
	...
	/*Si l’utilisateur demande la création d’un dépôt git sans préciser le langage du projet, le programme doit
	exécuter les opérations suivantes :
	— Afficher le message d’erreur suivant : "You must set project type, please check the help : initdev –help".*/
	if(language == NOT_SPECIFIED && git_is_chosen){
		message = strdup("\nYou must set project type, please check the help : initdev –help\n");
		return 0;
	}

	//ici normalement tout est bien !
	return 1;
}
